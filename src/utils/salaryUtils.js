import { cpfContribution, sdl as sdlLevy } from './cpfRates';
...
export const calcPayslip = ({ config, attendanceRecords, nplDays, month }) => {
  ...
  // CPF — age/residency-aware lookup (src/utils/cpfRates.js), driven by the
  // employee's dob/residency/prStartDate at the payslip month. `cpfApplicable`
  // remains a master on/off switch (e.g. a work-permit foreigner → no CPF).
  // asOf = first of the payslip month, in Singapore time.
  const asOf = new Date(`${month}-01T00:00:00+08:00`);
  const cpf = config.cpfApplicable
    ? cpfContribution({
        dob: config.dob,
        residency: config.residency ?? 'citizen',
        prStartDate: config.prStartDate,
        grossWage: grossPay,
        asOf,
      })
    : { employee: 0, employer: 0, cpfWage: 0 };
  const cpfWage      = cpf.cpfWage;
  const cpfEmployee  = cpf.employee;
  const cpfEmployer  = cpf.employer;
  // SDL (Skills Development Levy) — employer cost, always payable on wages
  // regardless of CPF applicability (covers foreign workers too).
  const sdl          = sdlLevy(grossPay);
  ...
  return {
    ...
    grossPay, cpfWage, cpfEmployee, cpfEmployer, sdl,
    // Total employer cost = gross + employer CPF + SDL (not deducted from staff).
    employerCost: Math.round((grossPay + cpfEmployer + sdl) * 100) / 100,
...
/* ── Print payslip — MOM-compliant itemised payslip ──────────────── */
// Carries all 12 items MOM requires on an itemised payslip:
//  1 employer name, 2 employee name, 3 date of payment, 4 basic salary,
//  5 salary period (start+end), 6 allowances itemised, 7 additional payments
//  (OT itemised), 8 deductions itemised, 9 overtime hours, 10 overtime pay,
//  11 net salary, 12 employer + employee CPF amounts.
const escapeHtml = (s) => String(s ?? '')
  .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
  .replace(/"/g, '&quot;');

export const printPayslip = (payslip, staffName, month) => {
  const fmtLong = (d) => new Intl.DateTimeFormat('en-SG', { timeZone: 'Asia/Singapore', day: '2-digit', month: 'short', year: 'numeric' }).format(d);
  ...
  // Item 5 — salary period start + end.
  const { from, to } = monthRange(month);
  const periodStart = fmtLong(new Date(`${from}T00:00:00+08:00`));
  const periodEnd   = fmtLong(new Date(`${to}T00:00:00+08:00`));
  // Item 3 — date of payment: stored value if present, else the print date.
  const payDate = payslip.paidOn
    ? fmtLong(new Date(`${String(payslip.paidOn).slice(0, 10)}T00:00:00+08:00`))
    : fmtLong(new Date());

  const bannerUrl = (typeof window !== 'undefined' ? window.location.origin : '') + banner;
  const hasDeductions = payslip.nplDeduction > 0 || payslip.cpfEmployee > 0 || (payslip.otherDeductions ?? []).length > 0;

  const html = `<!DOCTYPE html><html><head><meta charset="utf-8">
  <title>Payslip – ${escapeHtml(staffName)} – ${monthLabel}</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: Arial, sans-serif; font-size: 12px; color: #1a1a2e; padding: 32px; max-width: 640px; margin: 0 auto; }
    .header { display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 2px solid #CC0000; padding-bottom: 12px; margin-bottom: 6px; }
    .logo-wrap { background: #1a1a2e; border-radius: 8px; padding: 6px 10px; display: inline-flex; align-items: center; }
    .logo-img { height: 28px; width: auto; display: block; }
    .employer { font-size: 15px; font-weight: 700; color: #1a1a2e; margin-top: 8px; }
    .employer-sub { font-size: 11px; color: #5a6577; margin-top: 2px; }
    .payslip-title { font-size: 13px; font-weight: 600; color: #5a6577; }
    .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6px 20px; margin: 16px 0 20px; background: #f5f6f8; padding: 12px 14px; border-radius: 6px; }
    ...
    .employer-cost { font-size: 11px; color: #5a6577; margin-bottom: 16px; background: #f5f6f8; padding: 10px 12px; border-radius: 6px; }
    .employer-cost b { color: #1a2233; }
    ...
    .banner-img { display: block; margin: 22px auto 0; width: 100%; max-width: 200px; height: auto; }
    .note { font-size: 10px; color: #5a6577; margin-top: 14px; text-align: center; }
  </style></head><body>
  <div class="header">
    <div>
      <div class="logo-wrap"><img src="${LOGO_B64}" class="logo-img" alt="WA! Network Asia" /></div>
      <div class="employer">WA! NETWORK ASIA</div>
      <div class="employer-sub">CCTV Installation Contractor</div>
    </div>
    <div style="text-align:right"><div class="payslip-title">ITEMISED PAYSLIP</div><div style="font-size:11px;color:#5a6577;margin-top:2px;">${monthLabel}</div></div>
  </div>

  <div class="info-grid">
    <div class="info-row"><span class="info-lbl">Employee</span><span class="info-val">${escapeHtml(staffName)}</span></div>
    ${payslip.designation ? `<div class="info-row"><span class="info-lbl">Designation</span><span class="info-val">${escapeHtml(payslip.designation)}</span></div>` : ''}
    <div class="info-row"><span class="info-lbl">Salary Period</span><span class="info-val">${periodStart} – ${periodEnd}</span></div>
    <div class="info-row"><span class="info-lbl">Date of Payment</span><span class="info-val">${payDate}</span></div>
    ...
    ${(payslip.bankName || payslip.bankAccount) ? `<div class="info-row"><span class="info-lbl">Paid To</span><span class="info-val">${escapeHtml([payslip.bankName, payslip.bankAccount].filter(Boolean).join(' · '))}</span></div>` : ''}
  </div>

  <table>
    <tr><th colspan="2">EARNINGS</th></tr>
    <tr><td>Basic Salary</td><td>${fmtAmt(payslip.basicPay)}</td></tr>
    ${payslip.otPay > 0 ? `<tr><td>Overtime Pay — ${payslip.otHours}h @ ${payslip.otMultiplier ?? 1.5}×</td><td>${fmtAmt(payslip.otPay)}</td></tr>` : ''}
    ${(payslip.allowances ?? []).map(a => `<tr><td>${escapeHtml(a.name || 'Allowance')}</td><td>${fmtAmt(a.amount)}</td></tr>`).join('')}
    <tr class="subtotal"><td>Gross Salary</td><td>${fmtAmt(payslip.grossPay)}</td></tr>
  </table>
  ...
  <table>
    ...
    ${payslip.cpfEmployee > 0 ? `<tr><td>Employee CPF Contribution</td><td>– ${fmtAmt(payslip.cpfEmployee)}</td></tr>` : ''}
    ${(payslip.otherDeductions ?? []).map(d => `<tr><td>${escapeHtml(d.name || 'Deduction')}</td><td>– ${fmtAmt(d.amount)}</td></tr>`).join('')}
    ${!hasDeductions ? '<tr><td colspan="2" style="color:#5a6577">No deductions</td></tr>' : ''}
    <tr class="net-row"><td>NET SALARY</td><td>${fmtAmt(payslip.netPay)}</td></tr>
  </table>

  <div class="employer-cost">
    <div>Employee CPF contribution: <b>${fmtAmt(payslip.cpfEmployee)}</b></div>
    <div>Employer CPF contribution: <b>${fmtAmt(payslip.cpfEmployer)}</b> — not deducted from employee</div>
    ${payslip.sdl > 0 ? `<div>Skills Development Levy (SDL): <b>${fmtAmt(payslip.sdl)}</b> — employer cost</div>` : ''}
  </div>
